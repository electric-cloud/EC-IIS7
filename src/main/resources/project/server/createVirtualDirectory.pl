...
# -------------------------------------------------------------------------
  ...
  $::gPath = ($::gEC->getProperty("path") )->findvalue("//value");
  $::gPhysicalPath = ($::gEC->getProperty("physicalpath") )->findvalue("//value");
  ...
  sub main() {
    ...
    $cmdLine = "$appcmdLocation add vdir /app.name:\"$::gAppName\" /path:\"$::gPath\" /physicalPath:\"$::gPhysicalPath\"";
    ...
    if($? == SUCCESS){
        ...
        if($content !~ m/VDIR object "(.+)" added/){
