...
# -------------------------------------------------------------------------
  ...
  sub main() {
    ...
    $cmdLine = "$appcmdLocation add apppool /name:\"$::gAppPoolName\"";
    ...
    if($content !~ m/APPPOOL "$::gAppPoolName"/){
         ...
         print "App Pool $::gAppPoolName has not been created yet. Proceeding to adding it.\n";
         ...
         if($? == SUCCESS){
             ...
             if($content !~ m/APPPOOL object "(.+)" added/){
        ...
        print "App Pool $::gAppPoolName already exists.";
